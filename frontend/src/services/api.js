...
export const apiService = {
  ...
  // Get donations (for alumni - requires donor_id)
  async getDonations(donorId) {
    try {
      if (!donorId) {
        throw new Error('donor_id is required');
      }
      const url = `${API_BASE_URL}/api/donations?donor_id=${donorId}`;
      const response = await fetch(url);
  ...
  // Get all donations (for admin - with sorting)
  async getAdminDonations(sortBy = 'created_at', sortOrder = 'DESC') {
    try {
      const url = `${API_BASE_URL}/api/admin/donations?sortBy=${sortBy}&sortOrder=${sortOrder}`;
      const response = await fetch(url);
